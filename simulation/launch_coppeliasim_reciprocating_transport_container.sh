...
INNER_SCRIPT="${ROOT}/outputs/control_runs/_coppelia_reciprocating_container_inner.sh"
...
export RUN_SUFFIX=${RUN_SUFFIX:-coppelia_reciprocating}
export ACCEL_TORQUE_POLICY=${ACCEL_TORQUE_POLICY:-ik_joint_pd}
export RECIPROCATING_STROKE_M=${RECIPROCATING_STROKE_M:-0.018}
export RECIPROCATING_HOLD_S=${RECIPROCATING_HOLD_S:-0.35}
export A_X_MAX=${A_X_MAX:-0.03}
export V_X_MAX=${V_X_MAX:-0.018}
export SETTLE_DURATION=${SETTLE_DURATION:-2.0}
export DURATION=${DURATION:-0}
cd ${ROOT}
exec bash simulation/launch_coppeliasim_reciprocating_transport.sh "\$@"
EOF
