...
VIDEO_PATH="${ROOT}/demonstration_videos/ur5e_coppeliasim/coppeliasim_ur5_origin_acquisition.mp4"
SUMMARY_PATH="${STATE_DIR}/coppeliasim_ur5_origin_acquisition_summary.json"
...
SENSING_MARKER="${STATE_DIR}/ur5_origin_acquisition_addon_sensing.txt"
...
if ! compgen -G "${FRAME_DIR}/frame_*.png" > /dev/null; then
  echo "No origin acquisition video frames were captured." >&2
...
if [[ "${frame_count}" -lt "${FRAME_COUNT}" ]]; then
  echo "Only captured ${frame_count}/${FRAME_COUNT} origin acquisition video frames." >&2
  sed -n '1,260p' "${SIM_LOG}" >&2 || true
  exit 1
fi
...
if ! grep -q '"success": true' "${SUMMARY_PATH}" || ! grep -q '"base_on_ground": true' "${SUMMARY_PATH}"; then
  echo "Origin/acceleration guardrails failed:" >&2
  cat "${SUMMARY_PATH}" >&2
  exit 1
fi

echo "Saved origin acquisition video: ${VIDEO_PATH}"
echo "Saved origin acquisition summary: ${SUMMARY_PATH}"
